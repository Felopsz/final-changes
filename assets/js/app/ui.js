(function(){
  ...
  const { isTV, toggleTVMode } = APP.tv;
  ...
  function initDashboard(){
    ...
    function updateGreeting(){
      ...
      if (els.greet) els.greet.textContent = `${hello}, ${window.CURRENT_USER}`;
    }
    ...
    const UI = {
      ...
      async addTicketNote(t, text){
        await DB.addTicketNote(t.id, text, window.CURRENT_USER);
      },
      ...
      async addProjectNote(p, text){
        await DB.addProjectNote(p.id, text, window.CURRENT_USER);
      },
      ...
      openProjectDetailInline(p){
        ...
        els.projDetailsInline.innerHTML = `
          <div class="project-detail-inline panel">
            ...
            <div class="pbar mt-2"><i style="width:${p.pct}%"></i></div>
            <div class="subtabs">
              <button class="subtab active" data-tab="pdDesc" >Descrição</button>
              <button class="subtab" data-tab="pdNotes" >Anotações</button>
              <button class="subtab" data-tab="pdRDO" >RDO's</button>
        ...
        try {
          if (APP.helpers && typeof APP.helpers.renderProjectObservacoes === 'function') {
            APP.helpers.renderProjectObservacoes(p);
          } else {
            throw new Error('helper missing');
          }
        } catch(e){
          const wrap = document.getElementById('pdObs');
          if (wrap){
            let list = [];
            if (Array.isArray(p.observacoes)) list = p.observacoes;
            else if (Array.isArray(p.obs)) list = p.obs;
            else if (DB.state && DB.state.projectObs && Array.isArray(DB.state.projectObs[p.id])) list = DB.state.projectObs[p.id];
            const items = (list || []).map(o => {
              if (typeof o === 'string') return { text: o };
              if (o && typeof o === 'object') return { text: (o.text || o.msg || o.description || JSON.stringify(o)), date: o.date || o.when };
              return { text: String(o) };
            });
            wrap.innerHTML = items.length
              ? '<ul class="list-grid">' + items.map(i => `<li>${esc(i.text || '')}${i.date ? `<br><small>— ${new Date(i.date).toLocaleDateString('pt-BR')}</small>` : ''}</li>`).join('') + '</ul>'
              : '<p class="muted">Nenhuma observação.</p>';
          }
        }
    ...
    // Exponha para outros módulos (ex.: TV)
    APP.UI = UI;
  ...
  APP.dashboard = { init: initDashboard };
})();
